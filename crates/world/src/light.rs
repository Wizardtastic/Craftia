...
use crate::chunk::{pack_rgb, Chunk};
...
///
/// The cross-chunk callback now carries `(intensity, packed_rgba8)` so
/// **full** colored light propagates across chunk boundaries — no palette
/// quantization, no `LIGHT_COLORS` lookup. The packed u32 is the same
/// `(A<<24)|(B<<16)|(G<<8)|R` layout the vertex shader reads as
/// `R8G8B8A8_UNORM`.
pub fn compute_torchlight(
    ...
    cross_chunk_update: &mut dyn FnMut(BlockPos, u8, u32),
) {
    let origin = chunk_origin(chunk.pos);

    // Clear existing torchlight and color, then seed from emitters.
    for t in chunk.torchlight.iter_mut() {
        *t = 0;
    }
    for c in chunk.torchlight_color.iter_mut() {
        *c = 0;
    }
        ...
        for lz in 0..CHUNK_SIZE {
            for lx in 0..CHUNK_SIZE {
                ...
                if emission > 0 {
                    ...
                    // Use the emitter's actual RGB color, bit-packed for the
                    // vertex shader. This was previously snapped to the
                    // nearest of 16 palette colors which washed out distinct
                    // emissive tints (e.g. a `light_blue` block's pure
                    // `[60,120,255]` ended up looking like sea-lantern).
                    let ec = reg.emission_color(block);
                    chunk.torchlight_color[idx] = pack_rgb(ec[0], ec[1], ec[2]);
    ...
    while let Some((lx, ly, lz, level)) = queue.pop() {
        ...
        let new_level = level - 1;
        // Get the color of the source block we're propagating from.
        let src_idx = local_index(lx, ly, lz);
        let src_color = chunk.torchlight_color[src_idx];
        ...
        for &(dx, dy, dz) in &NEIGHBOURS {
                ...
                && (0..CHUNK_SIZE).contains(&nz)
            {
                ...
                if chunk.torchlight[nidx] < new_level {
                    ...
                    chunk.torchlight_color[nidx] = src_color;
                    queue.push(nx, ny, nz, new_level as i32);
                }
            } else {
                ...
                if (0..WORLD_HEIGHT_BLOCKS).contains(&wy) {
                    ...
                    if new_level > external {
                        cross_chunk_update(BlockPos::new(wx, wy, wz), new_level, src_color);
                    }
                }
            }
        }
    ...
    for ly in 0..cs {
        for lz in 0..cs {
            ...
            if lv0 > 0 {
                let color0 = chunk.torchlight_color[local_index(0, ly, lz)];
                let wx = origin.x - 1;
                let wy = origin.y + ly;
                let wz = origin.z + lz;
                if (0..WORLD_HEIGHT_BLOCKS).contains(&wy) {
                    cross_chunk_update(BlockPos::new(wx, wy, wz), lv0, color0);
                }
            }
            // x = CHUNK_SIZE-1 boundary
            let lv15 = chunk.torchlight[local_index(cs - 1, ly, lz)];
            if lv15 > 0 {
                let color15 = chunk.torchlight_color[local_index(cs - 1, ly, lz)];
                let wx = origin.x + cs;
                let wy = origin.y + ly;
                let wz = origin.z + lz;
                if (0..WORLD_HEIGHT_BLOCKS).contains(&wy) {
                    cross_chunk_update(BlockPos::new(wx, wy, wz), lv15, color15);
                }
            }
        }
    }
    for lx in 0..cs {
        for ly in 0..cs {
            ...
            if lv0 > 0 {
                let color0 = chunk.torchlight_color[local_index(lx, ly, 0)];
                let wx = origin.x + lx;
                let wy = origin.y + ly;
                let wz = origin.z - 1;
                if (0..WORLD_HEIGHT_BLOCKS).contains(&wy) {
            ...
            if lv15 > 0 {
                let color15 = chunk.torchlight_color[local_index(lx, ly, cs - 1)];
                let wx = origin.x + lx;
                let wy = origin.y + ly;
                let wz = origin.z + cs;
                if (0..WORLD_HEIGHT_BLOCKS).contains(&wy) {
...
mod tests {
    ...
    fn compute_sunlight_open_sky() {
        ...
        compute_sunlight(&mut chunk, &reg, Vec3::new(0.0, 1.0, 0.0), &air_sample);
    ...
    fn compute_sunlight_below_roof() {
        ...
        compute_sunlight(&mut chunk, &reg, Vec3::new(0.0, 1.0, 0.0), &sample_block_fn);
