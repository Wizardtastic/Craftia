...
pub struct BlockData {
    ...
    #[serde(default)]
    pub emission: u8,
    /// RGB color of emitted light: `[r, g, b]`. Default warm white if absent.
    #[serde(default)]
    pub emission_color: Option<[u8; 3]>,
    #[serde(default = "default_absorption")]
    pub light_absorption: u8,
    /// Optional per-block map color override: `[r, g, b, a]`.
    /// If absent, the world crate's `default_map_color` heuristic is used.
    #[serde(default)]
    pub map_color: Option<[u8; 4]>,
    /// Hardness: seconds to break with bare hand (default 1.5).
    #[serde(default = "default_hardness")]
    pub hardness: Option<f32>,
    /// Blast resistance for TNT calculations (default 6.0).
    #[serde(default = "default_blast_resistance")]
    pub blast_resistance: Option<f32>,
    /// Required tool type to harvest this block (e.g. "pickaxe", "axe", "shovel").
    /// If absent, any tool (or hand) can harvest.
    #[serde(default)]
    pub required_tool: Option<String>,
    /// Minimum tool tier required (0 = any, 1 = wood+, 2 = stone+, 3 = iron+, 4 = diamond+).
    #[serde(default)]
        ...
        if let Some(same) = value.get("same") {
            if same.is_u64() {
                ...
                return Err(de::Error::custom(
                    "field 'same' must be a number in 0..=65535",
                ));
            } else {
                return Err(de::Error::custom("field 'same' must be a number"));
            }
        }
        let pf: PerFaceFields = serde_json::from_value(value).map_err(de::Error::custom)?;
...
fn default_hardness() -> Option<f32> {
    Some(1.5)
}
fn default_blast_resistance() -> Option<f32> {
    Some(6.0)
}
