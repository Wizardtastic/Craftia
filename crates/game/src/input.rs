...
pub enum Action {
    ...
    Profiler,
    ChunkDebug,
    /// Toggle the runtime ECS inspector overlay (F10).
    EcsInspector,
    /// Toggle the developer console (` or F2).
    DeveloperConsole,
    /// Toggle the telemetry dashboard overlay (F12).
    TelemetryDashboard,
    /// Toggle world edit mode (X).
    EditMode,
    /// Toggle block palette (E, only when edit mode active).
    TogglePalette,
    /// Toggle fullscreen map (M).
    FullscreenMap,
    // --- Inspector extensions (entity pinning) ---
    /// Toggle pin: pin the player if nothing is pinned; clear otherwise.
    PinUnpin,
    /// Cycle forward through live ECS entities.
    PinCycleNext,
    /// Cycle backward through live ECS entities.
    PinCyclePrev,
    /// Explicit clear of the current pin.
    PinClear,
    // Combat / Mining
    /// Left mouse button held (mining/attacking).
    Attack,
    /// Right mouse button pressed (placing/using).
    Use,
}
