...
pub fn environmental_damage_system(world: &mut World, _dt: f32) {
    ...
    let health = world.get::<voxel_combat::Health>(player_entity).copied().unwrap_or_default();
    if health.dead {
        return;
    }
    ...
    if let Some(phys) = physics {
        ...
        if head_def.solid && !head_block_id.is_air() && head_def.kind != voxel_world::registry::BlockKind::Liquid {
            if let Some(dq) = world.resource_mut::<DamageQueue>() {
                dq.push(player_entity, DamageEvent {
