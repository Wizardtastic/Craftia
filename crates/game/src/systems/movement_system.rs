...
pub fn movement_system(world: &mut World, dt: f32) {
    ...
    if input.flying {
        ...
        // Minecraft-like movement: ground has acceleration + friction, air has weak control.
        if state.on_ground {
            if wish_horizontal.length_squared() > 1e-6 {
                // Ground: accelerate toward target speed.
                let target = wish_horizontal * speed;
                new_vel.x = approach(new_vel.x, target.x, dt * GROUND_ACCEL * speed);
                new_vel.z = approach(new_vel.z, target.z, dt * GROUND_ACCEL * speed);
            } else {
            ...
            if wish_horizontal.length_squared() > 1e-6 {
                let target = wish_horizontal * speed;
                new_vel.x = approach(new_vel.x, target.x, dt * AIR_CONTROL * speed);
                new_vel.z = approach(new_vel.z, target.z, dt * AIR_CONTROL * speed);
            }
            // No friction in air — keep horizontal velocity.
        }
    }
    ...
    state.eye_offset = if input.sneaking {
        EYE_HEIGHT_SNEAK
    } else {
        EYE_HEIGHT
    };
    ...
    // Update bob phase from horizontal speed (for camera bobbing).
    let horiz_speed = glam::Vec3::new(new_vel.x, 0.0, new_vel.z).length();
    if state.on_ground && horiz_speed > 0.1 && !input.sneaking && !in_water {
        let speed_factor = (horiz_speed / speed).min(1.0);
        let bob_freq = if input.sprinting { 18.0 } else { 12.0 };
        state.bob_phase += dt * speed_factor * bob_freq;
    } else {
        // Phase drifts toward 0 when not moving (for smooth transition).
        state.bob_phase *= 1.0 - 5.0 * dt;
    }

    // Decay mining swing timer.
    if state.mining_swing > 0.0 {
        state.mining_swing -= dt / 0.4; // 400ms swing
        if state.mining_swing < 0.0 {
            state.mining_swing = 0.0;
        }
    }
